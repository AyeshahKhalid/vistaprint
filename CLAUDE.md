...
- **Home page dimension-level Figma verification completed 2026-09-17** (method: extract exact `absoluteBoundingBox`/`fills`/`cornerRadius`/`strokes` from cached Figma JSON per section, diff directly against SCSS — not screenshot comparison). Found and fixed a systemic bug: several card-style sections (`collection-card`, `idea-card`, `ambassador-card`, `service-card`) were missing their white-bg + bordered-container styling entirely, rendering as loose image+text instead of Figma's consistent bordered-card pattern. Also fixed: signup-offer section background colors (the two instances need swapped/distinct fills), signup eyebrow color, input fill colors, `ugc-tile__image` aspect ratio (246x260, not square) and corner radius (10px not 8px). Hero, promo-banner, and brand-story sections were checked and already matched. **This same rigor has NOT yet been applied to the other 10 category pages** — they only had an earlier, coarser polish pass (colors/icons/text, not per-card structural dimensions). Recommend applying the same method there before considering them Figma-verified.
